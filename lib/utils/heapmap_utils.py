...
def generate_heatmap(bboxes, patch_size=320, stride=16):
    ...
    for single_patch_bboxes in bboxes:
        ...
        CenterNetHeatMap.generate_score_map(gt_scoremap, classes, wh, centers_int, 0.7)  # 生成的高斯热图放到gt_scoremap中
...
class CenterNetHeatMap(object):
    ...
    def generate_score_map(fmap, gt_class, gt_wh, centers_int, min_overlap):
        radius = CenterNetHeatMap.get_gaussian_radius(gt_wh, min_overlap)   # 生成高斯核的半径
        ...
        for i in range(gt_class.shape[0]):
            ...
            CenterNetHeatMap.draw_gaussian(fmap[channel_index], centers_int[i], radius[i])  # 根据中心点坐标和半径画出高斯热图，并放到fmap中
    ...
    def gaussian2D(radius, sigma=1):
        ...
        m, n = radius
        y, x = np.ogrid[-m: m + 1, -n: n + 1]   # 生成网格，y和x代表两个方向

        gauss = np.exp(-(x * x + y * y) / (2 * sigma * sigma))      # 对应原点
        gauss[gauss < np.finfo(gauss.dtype).eps * gauss.max()] = 0   #  将高斯核中值非常接近于零的元素设置为零
        return gauss

    @staticmethod
    def draw_gaussian(fmap, center, radius, k=1):
        diameter = 2 * radius + 1
        gaussian = CenterNetHeatMap.gaussian2D((radius, radius), sigma=diameter / 6)   # 生成二维高斯核，没有做归一化处理，只有中间的那个数为1
